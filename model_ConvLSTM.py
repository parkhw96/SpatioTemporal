...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        self.conv_output = nn.Conv2d(in_channels= 1 * HIDDEN_DIM,
                        out_channels = 1,
    ...
    def forward(self, input_tensor, hidden_state = None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            h, c = hidden_state[layer_idx]  # B x hid_dim x H x W
            ...
            for t in range(seq_len):
                ...
                if(layer_idx == (self.num_layers - 1)):  # processing if last layer
                    new_h = self.conv_output(h)
                    output_inner.append(new_h)

                else:
                    output_inner.append(h)

            layer_output = torch.stack(output_inner, dim=1)  # B x S x hid_dim x H x W
